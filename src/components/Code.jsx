...
function CodePanelHeader({ tag, label }) {
  ...
  return (
    <div className="flex h-9 items-center gap-2 border-y border-b-white/7.5 border-t-transparent bg-white/2.5 bg-zinc-900 px-4 dark:border-b-white/5 dark:bg-white/1">
      ...
      {label && (
        <div className="font-mono select-all text-xs text-zinc-400">{label}</div>
      )}
    </div>
  )
}
      ...
      {hasTabs && (
          <Tab.List className="-mb-px flex gap-4 text-xs font-medium">
            {Children.map(children, (child, childIndex) => (
                <Tab
                    className={clsx(
                        'mt-2 py-1 px-3 border rounded-full transition ui-not-focus-visible:outline-none',
                        childIndex === selectedIndex
                            ? 'border-emerald-500 text-emerald-400'
                            : 'border-transparent text-zinc-400 hover:text-zinc-300',
                        isValidElement(child) && child.props.className ? child.props.className : 'default-class'
                    )}
                >
